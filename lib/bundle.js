...
function arrayBufferToURI(bytes) {
	...
	var len = data.length,
	    index = 0;
	for (; index < len; index += 3) base64 += btoa(String.fromCharCode.apply(null, data.slice(index, index + 3)));
...
function logObj(name, value, layer, p) {
	...
	if (typeof value != "object") {
		p.innerHTML += indent + name + ": " + value + "<br />";
		return;
	}
	p.innerHTML += indent + name + ":" + "<br />";
	++layer;
	...
	render: function () {
		return React.createElement(
			...
			React.createElement(
				CardAction,
				{ expandable: true },
				React.createElement(FileChooseButton, { FileChange: this.handleMIMEChange })
			),
			React.createElement(
				CardText,
				{ expandable: true },
	...
	render: function () {
		return React.createElement(
			...
			React.createElement(
				Paper,
				{ style: { padding: '4px' } },
				React.createElement(HeadIntro, null)
			),
			React.createElement(
				Paper,
				{ style: { padding: '4px' } },
